...
/** Point entry form used during a live game. Keeps the last line prefilled and syncs to Firestore. */
export function PointLogger({
  ...
  const log = async () => {
    setIsSaving(true);
    const pointPayload: Omit<GamePoint, "id"> = {
      result,
      playerIds: [...playerIds],
      blocks,
      assists,
    ...
    try {
      // If gameId is passed, persist point record to Firestore cloud collection
      if (gameId) {
        await addDoc(collection(db, "games", gameId, "points"), {
          ...pointPayload,
          createdAt: serverTimestamp(),
        });
      }

      // Execute parent handler for local state updates
      onLog(pointPayload);
      ...
      console.error("Failed to log point to Firestore:", error);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="panel space-y-4 p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end">
        ...
        <button
          onClick={log}
          disabled={playerIds.length < 4 || isSaving}
